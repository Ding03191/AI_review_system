...
function openDrawer(){ drawer.classList.add('open'); scrim.hidden=false; btnOpenDrawer.setAttribute('aria-expanded','true'); drawer.setAttribute('aria-hidden','false'); }
function closeDrawer(){ drawer.classList.remove('open'); scrim.hidden=true; btnOpenDrawer.setAttribute('aria-expanded','false'); drawer.setAttribute('aria-hidden','true'); }
...
function initRoute(){ const name=(location.hash||'#teacher').replace('#',''); setActivePage(pages[name]?name:'teacher'); }
...
filesInput?.addEventListener('change',()=>{
  ...
  Array.from(filesInput.files).forEach((f,i)=>{
    const li=document.createElement('li'); li.textContent=`${i+1}. ${f.name} (${Math.round(f.size/1024)} KB)`; fileList.appendChild(li);
  });
});
...
document.getElementById('btnOcrGrid')?.addEventListener('click',async()=>{
  ...
  try{
    ...
    const f=j.fields||{}; const set=(n,v)=>{const el=formT.querySelector(`[name="${n}"]`); if(el&&v){el.value=v; el.dispatchEvent(new Event('change',{bubbles:true}));}};
    set('teacherName',f.teacher_name); set('teacherId',f.teacher_id); set('department',f.department);
    set('ext',f.ext); set('courseTitle',f.course_title); set('organizer',f.organizer);
    set('eventDate',f.event_date); set('startTime',f.start_time); set('endTime',f.end_time);
    previewEl.textContent=JSON.stringify(f,null,2);
  }catch(err){ msgEl.innerHTML='表格偵測錯誤：'+err.message; }
});
...
請判斷 1) 名稱一致 2) 時間要求 3) 總時數>=12 4) 測驗成績 5) 不得僅國高中生 6) 姓名一致 7) 至少一門符合特色 8) 去重，並以 JSON 回應。`;
  ...
  try{
    ...
    document.getElementById('feedbackWindow').textContent=JSON.stringify(data,null,2);
  }catch(err){
